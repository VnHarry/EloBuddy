...
namespace VnHarry_AIO.Utilities
{
    internal static class Utils
    {
        ...
        public static void DrawPolygon(this Geometry.Polygon polygon, Color color)
        {
            for (var i = 0; i < polygon.Points.Count; i++)
            {
                var lineStart = polygon.Points[i];
                var lineEnd = (i + 1 == polygon.Points.Count) ? polygon.Points[0] : polygon.Points[i + 1];
                Drawing.DrawLine(Drawing.WorldToScreen(lineStart.To3DPlayer()),
                    Drawing.WorldToScreen(lineEnd.To3DPlayer()), 1, color);
            }
        }
        ...
        public static void DrawPath(Vector2[] path, Color color)
        {
            for (var i = 0; i < path.Length - 1; i++)
            {
                var lineStart = path[i];
                var lineEnd = path[i + 1];
                Drawing.DrawLine(Drawing.WorldToScreen(lineStart.To3DWorld()),
                    Drawing.WorldToScreen(lineEnd.To3DWorld()), 1, color);
            }
        }
